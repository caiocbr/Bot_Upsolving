...
def add_user(sheet, sample_spread_sheet_id, user_name):
    ...
    user_exist = get_user_column_id(sheet, sample_spread_sheet_id, user_name)
    if(user_exist != None):
        ...
        users_quantity = 0
        cells = get_cells(sheet, sample_spread_sheet_id, range)
        for i in cells:
            for j in i:
                users_quantity = int(j)
        
        # Inserindo coluna no início
        insert_columns(sheet, sample_spread_sheet_id, UPSOLVING_TAB_ID, USERS_START_COLUMN_ID, 1)

        # Formatando a coluna caso seja o primeiro usuário a ser adicionado
        if users_quantity == 0:
            problems_pattern_format(
                sheet, sample_spread_sheet_id, UPSOLVING_TAB_ID,
                PROBLEMS_START_ROW, USERS_START_COLUMN_ID,
                problems_end_row, USERS_START_COLUMN_ID
            )
            change_column_size(sheet, sample_spread_sheet_id, UPSOLVING_TAB_ID, USERS_START_COLUMN_ID, 1, USERS_COLUMN_SIZE)
            user_pattern_format(sheet, sample_spread_sheet_id, UPSOLVING_TAB_ID, USERS_ROW, USERS_START_COLUMN_ID)
            centralize_cells(sheet, sample_spread_sheet_id, UPSOLVING_TAB_ID, USERS_ROW + 1, USERS_START_COLUMN_ID, problems_end_row, USERS_START_COLUMN_ID)
            bold_cells(sheet, sample_spread_sheet_id, UPSOLVING_TAB_ID, USERS_ROW, USERS_START_COLUMN_ID, PROBLEM_COUNTER_ROW, USERS_START_COLUMN_ID)
...
def delete_user(sheet, sample_spread_sheet_id, user_name):
    ...
    user_column_id = get_user_column_id(sheet, sample_spread_sheet_id, user_name)
    if(user_column_id == None):
        print("Usuário não encontrado")
    else:
        delete_columns(sheet, sample_spread_sheet_id, UPSOLVING_TAB_ID, user_column_id, 1)
