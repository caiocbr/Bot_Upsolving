...
def add_user(sheet, spread_sheet_id, tab_id, user_name):
    ...
    if(user_exist != None):
        ...
        insert_columns(sheet, spread_sheet_id, tab_id, USERS_START_COLUMN_ID, 1)

        # Formatando a coluna caso seja o primeiro usuário a ser adicionado
        if users_quantity == 0:
            format_problem_cells(
                sheet, spread_sheet_id, tab_id,
                PROBLEMS_START_ROW, USERS_START_COLUMN_ID,
                problems_end_row, USERS_START_COLUMN_ID
            )
            change_column_size(sheet, spread_sheet_id, tab_id, USERS_START_COLUMN_ID, 1, USERS_COLUMN_SIZE)
            format_user_cells(sheet, spread_sheet_id, tab_id, USERS_ROW, USERS_START_COLUMN_ID)
            centralize_cells(sheet, spread_sheet_id, tab_id, USERS_ROW + 1, USERS_START_COLUMN_ID, problems_end_row, USERS_START_COLUMN_ID)
            bold_cells(sheet, spread_sheet_id, tab_id, USERS_ROW, USERS_START_COLUMN_ID, PROBLEM_COUNTER_ROW, USERS_START_COLUMN_ID)
...
def delete_user(sheet, spread_sheet_id, tab_id, user_name):
    ...
    if(user_column_id == None):
        ...
        delete_columns(sheet, spread_sheet_id, tab_id, user_column_id, 1)
